...
from bert_embedder import (compute_input_arrays_tqa, 
						   compute_sentece_pair_embedding,
...
# answer columns
target_columns = train.columns[32:]
train_targets = train.loc[:, target_columns]

model = BERTEmbedder(bert_path=BERT_PATH)
train_tqa_bert_encoded = compute_sentence_embedding(train, "answer", 
                                                    model, device, bert_path=BERT_PATH)
...
def train_mlp(model_class, train_data, train_targets, 
	...
	for epoch in range(epochs):
		...
		valid_loss,valid_rho,valid_rhos = eval_epoch_mlp(valid_loader, model, device)
...
for fold, (train_idx, valid_idx) in enumerate(kf_split):
	...
	model,best_rho,best_rhos = train_mlp(OutputMLP, train_inputs, _train_targets,
...
train_inputs = compute_input_arrays(train, "answer", tokenizer, MAX_SEQUENCE_LENGTH)
...
for fold, (train_idx, valid_idx) in enumerate(kf_split):
	...
	model,best_rho,best_rhos = train_bert(model, _train_inputs, _train_targets, 
										  ...
										  device, patience=2, restore_best_state=True)
	kfold_rho.append(best_rho)
	kfold_rhos.append(best_rhos)
